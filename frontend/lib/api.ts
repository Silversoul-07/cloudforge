...
export async function getProfile(username: string): Promise<UserProfile|null> {     
    try{
      const Header = await getHeader();
      const response = await axiosServer.get(`/users/${username}`, Header);
      return response.data;
    }
    catch(error){
      return null;
...
export async function getUserCollections(username: string): Promise<CollectionData[]> {
  try {
    const response = await axiosServer.get(`/user/${username}/collections`);
    return response.data;
  } catch (error) {
